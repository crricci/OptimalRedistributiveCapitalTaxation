...
function main(args)
    ...
    case_name = "ipopt_tvc_lambda2c_T$(Int(round(T)))_N$(N)_beta$(replace(string(beta), "." => "p"))"
    ...
    run = _solveOptimalWealthTaxation(
        ...
        solve_kwargs = (; terminal_mode = :tvc_lambda2c, use_nested_seed = true, use_horizon_continuation = false),
    )

    tvc = OptimalWealthTax.transversality_metrics(run.result, run.params; terminal_mode = :tvc_lambda2c).terminal
    println((
        case = case_name,
