...
function main()
    ...
    case_dirs = sort(filter(name -> startswith(name, "ipopt_tvc_T") && isdir(joinpath(outputs_dir, name)), readdir(outputs_dir)))
    ...
    output_path = joinpath(outputs_dir, "ipopt_grid_beta075_summary.csv")
    headers = [
        "case_dir",
        "success",
        "residual_norm",
